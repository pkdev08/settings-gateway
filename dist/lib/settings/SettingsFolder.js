...
const arrayStrictEquals_1 = require("@klasa/utils/dist/lib/arrayStrictEquals");
...
class SettingsFolder extends Map {
     ...
     */
    toJSON() {
        return polyfills_1.fromEntries([...super.entries()].map(([key, value]) => [key, value instanceof SettingsFolder ? value.toJSON() : value]));
    }
    /**
    ...
    async _resolveFolder(context) {
        ...
        return polyfills_1.fromEntries(await Promise.all(promises));
    }
    ...
    _resetSettingsFolder(changes, schemaFolder, language, onlyConfigurable) {
        ...
        for (const entry of schemaFolder.values(true)) {
            ...
            const equals = entry.array ?
                arrayStrictEquals_1.default(previous, next) :
        ...
        for (const [path, value] of entries) {
            ...
            if (entry.type === 'Folder') {
                const keys = onlyConfigurable ?
                    [...entry.values()].filter(val => val.type !== 'Folder' && val.configurable).map(val => val.key) :
                    [...entry.keys()];
                throw keys.length > 0 ?
